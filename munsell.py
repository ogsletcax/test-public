...
from munsell_table import table
...
def _xyy_to_rgb_linear(x, y, y2):
    ...
    z2 = y2 * (1 - x - y) / y
    return (
        3.2406 * x2 - 1.5372 * y2 - 0.4986 * z2, -0.9689 * x2 + 1.8758 * y2 + 0.0415 * z2,
        ...
        if value > 10 or value < 0:
            if direction_changed:
                raise NotInTableError(
                    'Trying to interpolate, but cannot find'
...
class MunsellColor(object):
    ...
    def _to_rgb_interpolate(self):
        ...
        a, ad = self._interpolate_munsell(
            MunsellColor(ha, value(ha, mv, ca, 1), ca), MunsellColor(hb, value(hb, mv, ca, 1), ca))
        ...
        c, cd = self._interpolate_munsell(
            MunsellColor(ha, value(ha, mv, cb, 1), cb), MunsellColor(hb, value(hb, mv, cb, 1), cb))
